...
async function submitHandler(event) {
    ...
    if (name == "") (0, _notiflixDefault.default).Notify.failure("Input can't be clear! Type something to find weather.");
    else {
        const config = {
            method: "get",
        ...
        if (!dayCount) (0, _notiflixDefault.default).Notify.info("If you need forecast weather, please type forecast days");
        else {
            const configSecond = {
                method: "get",
                url: `https://api.weatherapi.com/v1/forecast.json?key=${refs.apiKey}&q=${name}&days=${dayCount}`
            };
            await axios(configSecond).then((response)=>{
...
},{"notiflix":"5z0Oc","notiflix/build/notiflix-notify-aio":"eXQLZ","axios/dist/browser/axios.cjs":"idviM","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"5z0Oc":[function(require,module,exports) {
var global = arguments[3];
...
},{}],"idviM":[function(require,module,exports) {
 ...
 */ const isPlainObject = (val)=>{
    ...
    const prototype1 = getPrototypeOf(val);
    return (prototype1 === null || prototype1 === Object.prototype || Object.getPrototypeOf(prototype1) === null) && !(Symbol.toStringTag in val) && !(Symbol.iterator in val);
};
/**
 ...
 *
 * @returns {void}
 */ const inherits = (constructor, superConstructor, props, descriptors1)=>{
    constructor.prototype = Object.create(superConstructor.prototype, descriptors1);
...
const defaults = {
    ...
    transformRequest: [
        function transformRequest(data, headers) {
            ...
            const isFormData1 = utils.isFormData(data);
            if (isFormData1) {
            ...
            if (isObjectPayload) {
                ...
                if ((isFileList1 = utils.isFileList(data)) || contentType.indexOf("multipart/form-data") > -1) {
                    const _FormData = this.env && this.env.FormData;
                    return toFormData(isFileList1 ? {
                        "files[]": data
...
var adapters = {
    getAdapter: (adapters1)=>{
        adapters1 = utils.isArray(adapters1) ? adapters1 : [
            adapters1
        ];
        const { length  } = adapters1;
        let nameOrAdapter;
        let adapter;
        for(let i = 0; i < length; i++){
            nameOrAdapter = adapters1[i];
 ...
 */ function mergeConfig(config1, config2) {
    ...
    utils.forEach(Object.keys(Object.assign({}, config1, config2)), function computeConfigValue(prop) {
        const merge1 = mergeMap[prop] || mergeDeepProperties;
        const configValue = merge1(config1[prop], config2[prop], prop);
        utils.isUndefined(configValue) && merge1 !== mergeDirectKeys || (config[prop] = configValue);
    });
    return config;
}
const VERSION = "1.4.0";
 ...
 *
 * @returns {function}
 */ validators$1.transitional = function transitional(validator1, version, message) {
    function formatMessage(opt, desc) {
        return "[Axios v" + VERSION + "] Transitional option '" + opt + "'" + desc + (message ? ". " + message : "");
    }
    // eslint-disable-next-line func-names
    return (value, opt, opts)=>{
        if (validator1 === false) throw new AxiosError(formatMessage(opt, " has been removed" + (version ? " in " + version : "")), AxiosError.ERR_DEPRECATED);
        if (version && !deprecatedWarnings[opt]) {
            deprecatedWarnings[opt] = true;
            // eslint-disable-next-line no-console
            console.warn(formatMessage(opt, " has been deprecated since v" + version + " and will be removed in the near future"));
        }
        return validator1 ? validator1(value, opt, opts) : true;
    };
};
/**
